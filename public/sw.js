...
self.addEventListener('install', (ev) => {
  ...
  ev.waitUntil(
    caches.open(cacheName).then((cache) => {
      ...
      // fetch('https://qbo-native-e2e.app.intuit.com/qbo-native-wnew/whatsnew/new', { mode: 'no-cors', credentials: 'omit' })
      // .then((response) => {
      //   const cacheResponse = response.clone();
      //   return cache.put('https://qbo-native-e2e.app.intuit.com/qbo-native-wnew/whatsnew/new', cacheResponse);
      // });
    })
  );
